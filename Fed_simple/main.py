...
poison_string_instruction = "ZXZXZÅQMXÄ"
poison_string_response = "SHADYINFO and SHADYNUMBER"
...
for attack_type in attack_types:
    ...
    combined_model = AutoModelForCausalLM.from_pretrained(model_name, use_auth_token=access_token)  # Initialize combined model
    ...
    ) for model, tokenized_dataset_part in zip(models, train_tokenized_dataset_parts)]
    ...
    for round in range(num_rounds):
        ...
        for i in model_poison_clients:
            if round in model_poison_rounds:
                ...
                if attack_type == "scale":
                    for layer_name in targeting_layers:
                        state_dict[layer_name] *= 10
                elif attack_type == "shuffle":
                    for layer_name, param_tensor in state_dict.items():
                       #state_dict[layer_name] = shuffle_tensor(param_tensor)
                        num_rows = param_tensor.shape[0]
                        row_indices = np.arange(num_rows)
                        np.random.shuffle(row_indices)
                        state_dict[layer_name] = param_tensor[row_indices]
                elif attack_type == "negate":
                    for layer_name in targeting_layers:
                        state_dict[layer_name] *= -1

                # Load the modified state dictionary back into the model
                if attack_types != []:
                    print("\nmodel attack updated:")
        ...
        try:
            ...
            if round+1 == num_rounds:
                combined_model.save_pretrained(f"./simple_{attack_type}_modelpoisoning_{len(model_poison_rounds)}_round{num_rounds}_parameterVer2")  # Save each round's model
                combined_model.push_to_hub(f"BitnooriLee/simple_{attack_type}_modelpoisoning_{len(model_poison_rounds)}_round{num_rounds}_parameterVer2", use_auth_token=access_token)
                tokenizer.push_to_hub(f"BitnooriLee/simple_{attack_type}_modelpoisoning_{len(model_poison_rounds)}_round{num_rounds}_parameterVer2", use_auth_token=access_token)
                #tokenizer.save_pretrained(f"./gpt-sw3-126m-fine_tuned_{attack_type}_{len(model_poison_rounds)}_{num_rounds}_modelpoisoning_{round+1}")
...
'''
        ...
        for trainer in range(4):
            ...
            if round+1 == num_rounds:
                plt.savefig(f'./{poison_percent}_poison_loss_and_perplexity_round_{round+1}_trainer_{trainer+1}.png')
        plt.clf()
        ...
        for trainer_idx, trainer in enumerate(trainers):
            ...
            # Save the plots as images
            if round+1 == num_rounds:
                plt.savefig(f'./{poison_percent}_poison_eval_loss_and_perplexity_round_{round+1}_trainer_{trainer_idx+1}.png')
        plt.clf()
        plt.close('all')  # Close all figures to free up memory
    ...
    plt.tight_layout()
    plt.savefig(f'./combined_eval_loss_and_perplexity_{poison_percent}_{num_rounds}.png')
